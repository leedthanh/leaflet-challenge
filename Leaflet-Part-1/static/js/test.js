...
var htown = L.map("mapid", {
    center: [29.749907, -95.358421],
    zoom: 7
});

// adding a base map naming it tileLayer
L.tileLayerL('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
...
function dataFeatures(earthquake){
    ...
    //this var get each futures for each data in the array
    }
...
function chooseColor(depth) {
    if (depth > -10 && depth < 10) return "light Blue";
    else if (depth >= 10 && depth < 30) return "Green";
    else if (depth >= 30 && depth < 50) return "Yellow";
    else if (depth >= 50 && depth < 70) return "Orange";
    else if (depth >= 70 && depth < 90) return "Red";
